...
VantageStarter is a production-ready SaaS boilerplate built on Next.js 15 + Convex + Clerk + Polar + AI SDK v6. Its UI is shadcn/ui (with lucide-react icons) as the primary layer, plus lit-ui web components on specific surfaces. It ships with a pre-configured agent development team in `.claude/agents/`, so every developer who clones this repo gets a specialist AI team on day one.
...
- shadcn/ui (`components/ui/`) with lucide-react icons is the primary UI layer; lit-ui web components (`src/components/ui/`, `lui-*` tags) are used on specific surfaces (landing, dashboard widgets). Match the pattern already present in the file you edit.
...
| UI Components | **shadcn/ui** (`components/ui/`, lucide-react icons) primary; **lit-ui** Lit.js v3 web components (`src/components/ui/`, `lui-*` tags) on landing + dashboard surfaces; Tailwind CSS v4 |
...
The primary UI layer is **shadcn/ui** in `components/ui/` (React components, lucide-react icons) — most dashboard, account, admin, and shared UI is built here. In parallel, **lit-ui** web components ship on specific surfaces (landing, some dashboard widgets).

### lit-ui integration (secondary layer)

lit-ui is a framework-agnostic web component library. Its integration in this repo:

- **Component source:** `src/components/ui/` — lit-ui `.ts` files
- **Pre-build:** `scripts/build-litui.mjs` (esbuild) → `public/lit-ui/bundle.js`
- **Shims:** `src/lib/lit-ui/` — core.ts, floating.ts, calendar.ts, tailwind-element.ts
- **Registration:** `src/components/ui/register-all.ts`
- **Loaders:** `components/LitUILoader.tsx` and `components/landing/WebComponentsLoader.tsx` — client-side
- **JSX types:** declared for `lui-*` tags
- **Alias:** `ui-button` (not `lui-button`) — keep this convention where lit-ui is used
...
components/             # Shared UI
  ui/                   # shadcn/ui components (.tsx, lucide-react icons)
...
src/
  ...
  lib/lit-ui/           # lit-ui shims (core, floating, calendar)
scripts/                # Build scripts (build-litui.mjs)
styles/presets/         # OKLCH color presets
.claude/
  agents/               # specialist agents
  skills/               # design + workflow skills
docs/
...
- **shadcn/ui + lucide-react is the primary UI layer;** lit-ui web components on landing/dashboard surfaces. Match the file you edit.
