...
- **2026-03-21** — lit-ui build integration — esbuild pre-build compiles Lit web components to `public/lit-ui/bundle.js` (525kb, 400ms) (`595ab6c`). **Context**: Landing page and dashboard components were using shadcn Button, shadcn Accordion, shadcn Skeleton/Badge, and lucide-react icons. These are now replaced with lit-ui web components (`<ui-button>`, `<lui-accordion>`, `<lui-accordion-item>`) and inline SVGs, reducing shadcn surface area and aligning the codebase with the lit-ui design system. **Added — 6 items**: (1) `scripts/build-litui.mjs` — standalone esbuild bundler for Lit web components. (2) `@lit-ui/core` shim (`TailwindElement`, `dispatchCustomEvent`). (3) `@lit-ui/core/floating` shim (wraps `@floating-ui/dom`). (4) `@lit-ui/calendar` shim (wraps `date-fns`). (5) `LitUILoader` client component — injects bundle in root layout. (6) `types/web-components.d.ts` — JSX type declarations for 22 web component tags. Also added: `prebuild` script in `package.json`; `@floating-ui/dom` and `@lit/task` dependencies. **Changed — landing page**: Hero, Nav, and Pricing CTAs now use `<ui-button>` (was shadcn `Button`); FAQ section now uses `<lui-accordion>` + `<lui-accordion-item>` (was shadcn `Accordion`); all lucide-react icons replaced with inline SVGs. **Changed — dashboard**: Main page, sidebar, user nav, architect, missions, and account pages migrated from shadcn to lit-ui; skeleton components replaced with CSS `animate-pulse` divs; all lucide-react icons replaced with inline SVGs. **Changed — infrastructure**: 22 lit-ui components now registered via `customElements.define()` (were exported but never registered); `tailwind-element.ts` CSS import path fixed (`../styles/tailwind.css` → `./tailwind.css`). **Removed**: shadcn Button imports from landing page components; shadcn Skeleton/Badge imports from dashboard pages; lucide-react icon imports from landing and dashboard pages.
