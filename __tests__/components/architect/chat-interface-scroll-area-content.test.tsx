 ...
 * (docs/migration-base-ui.md). `ChatInterface` is one of `scroll-area.tsx`'s
 * three consumers — this test proves the real chat message bubbles render
 * as content correctly nested inside the migrated
 * `data-slot="scroll-area-viewport"` element (Base UI requires an explicit
 * `ScrollArea.Content` wrapper between `Viewport` and children, unlike
 * Radix which had no such part), not merely that `ChatInterface` mounts.
 */
...
// jsdom does not implement scrollIntoView; ChatInterface's sentinel-follow
// effect calls it on mount.
Element.prototype.scrollIntoView = jest.fn();

import { ChatInterface } from "@/app/[locale]/dashboard/architect/_components/chat-interface";

describe("ChatInterface (ScrollArea migrated to Base UI)", () => {
	test("renders real message bubbles nested inside the scroll-area viewport", () => {
		...
		const userMessage = screen.getByText("Build me a landing page");
		const assistantMessage = screen.getByText("Here is the plan so far...");
