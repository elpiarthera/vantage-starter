...
interface _GpuChartInstance {
	...
	appendData(
		seriesIndex: number,
		newPoints: ReadonlyArray<readonly [number, number]>,
	): void;
}
...
export class LuiLineChart extends BaseChartElement {
	...
	private _flushLineUpdates(): void {
		...
		const seriesCount = Array.isArray(this.data)
			? (this.data as unknown[]).length
			: 1;
		const seriesUpdates = this._lineBuffers
			.slice(0, seriesCount)
			.map((buf) => ({ data: new Float32Array(buf as number[]) }));

		this._chart.setOption({ series: seriesUpdates }, {
			lazyUpdate: true,
		} as object);
...
if (
	...
	!customElements.get("lui-line-chart")
) {
