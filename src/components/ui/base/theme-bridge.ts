...
export class ThemeBridge {
	...
	readToken(name: string): string {
		...
		return (
			getComputedStyle(this.host).getPropertyValue(name).trim() || fallback
		);
	}
	...
	buildThemeObject(): object {
		return {
			...
			categoryAxis: {
				axisLine: {
					lineStyle: { color: this.readToken("--ui-chart-axis-line") },
				},
			...
			valueAxis: {
				...
				axisLabel: { color: this.readToken("--ui-chart-axis-label") },
				splitLine: {
					lineStyle: { color: this.readToken("--ui-chart-grid-line") },
				},
			},
			tooltip: {
