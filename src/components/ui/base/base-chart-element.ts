...
import { css, html, isServer, type PropertyValues } from "lit";
...
export abstract class BaseChartElement extends TailwindElement {
			...
			if (dom) {
				...
				for (const canvas of Array.from(canvases)) {
					const gl =
						...
						(
							canvas.getContext as (ctx: string) => WebGLRenderingContext | null
						)("webgl");
	...
	private _flushPendingData(): void {
		...
		if (this._streamingMode === "appendData") {
			...
			this._chart.setOption({ series: [{ data: this._circularBuffer }] }, {
				lazyUpdate: true,
			} as object);
		}
	}
	...
	protected async _detectRenderer(): Promise<void> {
		...
		if (isServer || typeof navigator === "undefined" || !navigator.gpu) {
			...
			dispatchCustomEvent(this, "renderer-selected", {
	...
	private async _maybeLoadGl(): Promise<void> {
		...
		// @ts-expect-error: echarts-gl 2.0.9 does not ship subpath type declarations.
