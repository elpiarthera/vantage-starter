...
export class TimeDropdown extends TailwindElement {
	...
	private _generateTimeOptions(): TimeOption[] {
		...
		const maxMinutes = maxParsed
			? maxParsed.hour * 60 + maxParsed.minute
			: 24 * 60 - 1;
	...
	private _scrollToSelected(): void {
		...
		requestAnimationFrame(() => {
			const idx = this.options.findIndex(
				(opt) =>
					opt.value.hour === this.value!.hour &&
					opt.value.minute === this.value!.minute,
			);
			if (idx < 0) return;
	...
	private _scrollHighlightedIntoView(): void {
		...
		requestAnimationFrame(() => {
			const optionEl = this._wrapperEl.children[this.highlightedIndex] as
				| HTMLElement
				| undefined;
			if (optionEl) {
				optionEl.scrollIntoView({ block: "nearest" });
