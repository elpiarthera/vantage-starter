...
import { type CSSResultGroup, css, html, isServer, nothing, svg } from "lit";
...
export class TimeVoiceInput extends TailwindElement {
  ...
  `;
	...
	private _parseVoiceTranscript(text: string): TimeValue | null {
		...
		if (hourMatch) {
			return this._buildTimeValue(parseInt(hourMatch[1], 10), 0, hourMatch[2]);
		}
	...
	protected render() {
      ...
      <div class="voice-input-wrapper">
        <button
          ...
          aria-label=${
						this._listening ? "Listening... tap to cancel" : "Voice input"
					}
