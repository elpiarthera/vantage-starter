...
import { columnPickerStyles, renderColumnPicker } from "./column-picker.js";
import {
	clearPreferences,
	loadPreferences,
	savePreferences,
...
> extends TailwindElement {
	 ...
	 */
	private tableId =
		`lui-data-table-${Math.random().toString(36).substring(2, 11)}`;
	...
	protected override updated(changedProperties: PropertyValues): void {
		...
		if (
			changedProperties.has("columnFilters") ||
			changedProperties.has("globalFilter")
		) {
			...
			if (
				...
				currentFilterState !== this._previousFilterState
			) {
		...
		if (
			changedProperties.has("dataCallback") &&
			this.dataCallback &&
			!changedProperties.get("dataCallback")
		) {
	...
	private async fetchData(): Promise<void> {
		...
		try {
			const result = await this.dataCallback(
				params,
			...
			if (result.pageCount !== undefined) {
				...
				this.pageCount = Math.ceil(
					result.totalRowCount / this.pagination.pageSize,
				);
			}
	...
	private isInteractiveElement(el: HTMLElement): boolean {
		...
		return (
			interactiveTags.includes(el.tagName) || el.hasAttribute("contenteditable")
		);
	}
	...
	private updateRovingTabindex(activePos: GridPosition): void {
		...
		const allCells = this.shadowRoot?.querySelectorAll(
	 ...
	 */
	private dispatchColumnVisibilityChange(
		columnVisibility: VisibilityState,
	): void {
	...
	private dispatchPreferenceChange(): void {
		...
		this._preferenceSaveTimer = setTimeout(() => {
			...
			this.dispatchEvent(
				new CustomEvent("ui-column-preferences-change", {
					detail: event,
					bubbles: true,
					composed: true,
				}),
	...
	public resetColumnPreferences(): void {
		...
		this.dispatchEvent(
			new CustomEvent("ui-column-preferences-reset", {
	...
	private saveRowEdit(): void {
		...
		const row = table
			.getRowModel()
			.rows.find((r) => r.id === this._editingRow?.rowId);
		if (!row) return;
		...
		if (this.bulkActions.length > 0 && selectedCount > 0) {
			...
			return renderBulkActionsToolbar(
				...
				showSelectAll
					...
					: undefined,
				showSelectAll
					? () => this.handleSelectAll(table, totalCount)
					: undefined,
			);
		}
	...
	private renderResizeHandle(header: Header<TData, unknown>): TemplateResult {
		...
		const headerLabel =
			typeof columnHeader === "string" ? columnHeader : "column";
	...
	private renderCell(
		...
		const isCellEditing =
			this._editingCell?.rowId === cell.row.id &&
		...
		if (isRowEditing && isEditable) {
			...
			const pendingValue =
				this._editingRow?.pendingValues[cell.column.id] ?? cell.getValue();
          ...
          ${renderEditInput(
						...
						},
						{
							onCommit: (value: unknown) =>
								this.updateRowEditValue(cell.column.id, value),
												...
												isRowEditing
													...
													: this.rowActions.length > 0
														? renderRowActions(
																...
																this.rowActions,
																(actionId, r) =>
																	this.handleRowAction(actionId, r),
															)
	...
	override render() {
		...
		this._tableInstance = this.tableController.table({
			columns: effectiveColumns,
			data: this.data,
			state: {
				sorting: this.sorting,
			...
			onColumnSizingInfoChange: (updater) => {
				const newInfo =
					typeof updater === "function"
						? updater(this._columnSizingInfo)
						: updater;
			...
			onRowSelectionChange: (updater) => {
				...
				this.rowSelection = newSelection;
				this.dispatchSelectionChange(this._tableInstance, newSelection, "user");
			},
			onColumnFiltersChange: (updater) => {
				...
				if (newFilters.length !== this.columnFilters.length) {
					...
					for (let i = 0; i < newFilters.length; i++) {
						const oldFilter = this.columnFilters.find(
							(f) => f.id === newFilters[i].id,
						);
						if (
							!oldFilter ||
							JSON.stringify(oldFilter.value) !==
								JSON.stringify(newFilters[i].value)
						) {
			...
			getCoreRowModel: getCoreRowModel(),
			getSortedRowModel: this.manualSorting ? undefined : getSortedRowModel(),
			getFilteredRowModel: this.manualFiltering
				? undefined
				: getFilteredRowModel(),
			getPaginationRowModel: this.manualPagination
				? undefined
				: getPaginationRowModel(),
						...
						onExpandedChange: (updater: any) => {
							const next =
								typeof updater === "function"
									...
									: updater;
							this.expanded = this.singleExpand
								? this.enforceSingleExpand(next)
								: next;
