...
export class KeyboardNavigationManager {
	...
	setBounds(bounds: GridBounds): void {
		...
		this.position.row = Math.min(
			this.position.row,
			Math.max(0, bounds.rowCount - 1),
		);
		this.position.col = Math.min(
			this.position.col,
			Math.max(0, bounds.colCount - 1),
		);
	}
	...
	private moveCol(delta: number): GridPosition {
		const newCol = Math.max(
			0,
			Math.min(this.position.col + delta, this.bounds.colCount - 1),
		);
	...
	private moveRow(delta: number): GridPosition {
		const newRow = Math.max(
			0,
			Math.min(this.position.row + delta, this.bounds.rowCount - 1),
		);
