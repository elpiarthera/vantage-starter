...
interface _GpuCandlestickInstance {
	...
	appendData(
		seriesIndex: number,
		newPoints: ReadonlyArray<readonly [number, number, number, number, number]>,
	): void;
}
...
export class LuiCandlestickChart extends BaseChartElement {
	...
	@property({ attribute: "moving-averages" }) movingAverages: string | null =
		null;
	...
	private _resolveMAColors(mas: MAConfig[]): string[] {
		...
		return mas.map((ma) => {
			...
			const token =
				LuiCandlestickChart._MA_DEFAULT_COLOR_TOKENS[
	...
	private async _initWebGpuLayer(): Promise<void> {
		...
		if (!adapter) {
			this._gpuChart = (await ChartGPU.create(
				gpuHost,
				gpuSeries,
			)) as unknown as _GpuCandlestickInstance;
		} else {
			this._gpuChart = (await ChartGPU.create(gpuHost, gpuSeries, {
				device,
				adapter,
			})) as unknown as _GpuCandlestickInstance;
		}
		...
		this._gpuResizeObserver = new ResizeObserver(() =>
			this._gpuChart?.resize(),
		);
	...
	private _syncCoordinates(): void {
		...
		const dataZoom = (
			option.dataZoom as Array<Record<string, unknown>> | undefined
		)?.[0];
		if (!dataZoom) return;
	...
	protected override _applyData(): void {
		...
		this._ohlcBuffer = this.data
			? [...(this.data as CandlestickBarPoint[])]
			: [];
		...
		const option = buildCandlestickOption(this._ohlcBuffer, {
			bullColor: this._wasWebGpu
				? "transparent"
				: (this.bullColor ?? undefined),
			bearColor: this._wasWebGpu
				? "transparent"
				: (this.bearColor ?? undefined),
	...
	override pushData(point: unknown): void {
		...
		if (this._ohlcBuffer.length > this.maxPoints) {
			...
			this._maStateMachines.forEach((sm) => {
				sm.trim(this.maxPoints);
			});
...
if (
	...
	!customElements.get("lui-candlestick-chart")
) {
