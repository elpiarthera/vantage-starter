...
async function runTests(): Promise<void> {
	...
	{
		const sm = new (
			MAStateMachine as new (config: {
				period: number;
				color: string;
				type?: "sma" | "ema";
			}) => typeof MAStateMachine.prototype
		)({ period: 3, color: "#fff" });
	...
	// SMA(3): NaN gap — [1, 2, NaN, 3, 4] → [null, null, null, 2.0, 3.0]
	{
		...
		assertApprox(
			result[3] as number,
			2.0,
			"SMA NaN-gap index 3 = 2.0 (window: 1,2,3)",
		);
		assertApprox(
			result[4] as number,
			3.0,
			"SMA NaN-gap index 4 = 3.0 (window: 2,3,4)",
		);
	...
	// SMA push() incremental — returns updated values array
	{
		...
		assertApprox(
			arr[4] as number,
			40,
			"SMA push(50) at index 4 = (30+40+50)/3 = 40",
		);
	...
	// valid closes: 1,3,4 → seed=(1+3+4)/3=2.67, then 5*0.5+2.67*0.5=3.83, 6*0.5+3.83*0.5=4.92
	{
		...
		assertNull(
			result[1],
			"EMA NaN-gap NaN → null (MA-03, no warmup increment)",
		);
		assertNull(result[2], "EMA NaN-gap index 2 null (only 2 valid: 1,3)");
		assertApprox(
			result[3] as number,
			2.667,
			"EMA NaN-gap index 3 = seed (1+3+4)/3=2.667",
		);
		assertApprox(
			result[4] as number,
			3.833,
			"EMA NaN-gap index 4 = 5*0.5+2.667*0.5=3.833",
		);
		assertApprox(
			result[5] as number,
			4.917,
			"EMA NaN-gap index 5 = 6*0.5+3.833*0.5=4.917",
		);
	...
	// values getter returns the same array reference (no copy)
	{
		...
		if (ref1 !== ref2) {
			throw new Error(
				"FAIL: values getter must return same array reference (no copy)",
			);
		}
