...
export class TimePicker extends TailwindElement {
	 ...
	 */
	private pickerId =
		`lui-time-picker-${Math.random().toString(36).substr(2, 9)}`;
  ...
  `;
	...
	private get showPresets(): boolean {
		return (
			this.presets === true ||
			(Array.isArray(this.presets) && this.presets.length > 0)
		);
	}
	...
	private focusTimeInput(): void {
		...
		if (timeInput?.shadowRoot) {
			const hourSpinbutton = timeInput.shadowRoot.querySelector(
				'[role="spinbutton"]',
			) as HTMLElement | null;
	...
	private validate(): boolean {
		...
		if (this.value) {
			...
			if (
				this.minTime &&
				!isEndTimeAfterStart(this.minTime, this.value, this.allowOvernight)
			) {
				const minParsed = parseTimeISO(this.minTime);
