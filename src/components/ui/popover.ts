...
export class Popover extends TailwindElement {
	...
	private async updatePosition(): Promise<void> {
		...
		const arrowEl =
			this.renderRoot.querySelector<HTMLElement>(".popover-arrow");
	...
	private startAutoUpdate(): void {
		...
		if (!this.triggerEl || !panel) return;
		this.cleanupAutoUpdate?.();
		this.cleanupAutoUpdate = autoUpdatePosition(this.triggerEl, panel, () =>
			this.updatePosition(),
		);
	}
	...
	private getFocusableElements(): HTMLElement[] {
		...
		const contentSlot = panel.querySelector<HTMLSlotElement>(
			'slot[name="content"]',
		);
