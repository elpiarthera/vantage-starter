...
describe("formatISOInterval", () => {
	it("formats a complete range as ISO interval", () => {
		expect(formatISOInterval("2026-01-10", "2026-01-20")).toBe(
...
describe("isDateInPreview", () => {
	it("returns true for date between start and hovered (hovered after start)", () => {
		expect(isDateInPreview("2026-01-15", "2026-01-10", "2026-01-20")).toBe(
			true,
		);
	});

	it("returns true for date between start and hovered (hovered before start)", () => {
		expect(isDateInPreview("2026-01-08", "2026-01-10", "2026-01-05")).toBe(
			true,
		);
	});

	it("returns true for date on start boundary", () => {
		expect(isDateInPreview("2026-01-10", "2026-01-10", "2026-01-20")).toBe(
			true,
		);
	});

	it("returns true for date on hovered boundary", () => {
		expect(isDateInPreview("2026-01-20", "2026-01-10", "2026-01-20")).toBe(
			true,
		);
	});

	it("returns false for date outside preview range", () => {
		expect(isDateInPreview("2026-01-25", "2026-01-10", "2026-01-20")).toBe(
			false,
		);
	});
...
describe("normalizeRange", () => {
	it("returns dates in order when already correct", () => {
		expect(normalizeRange("2026-01-10", "2026-01-20")).toEqual([
			"2026-01-10",
			"2026-01-20",
		]);
	});
	...
	it("swaps dates when end is before start", () => {
		expect(normalizeRange("2026-01-20", "2026-01-10")).toEqual([
			"2026-01-10",
			"2026-01-20",
		]);
	});
	...
	it("returns same dates when they are equal", () => {
		expect(normalizeRange("2026-01-15", "2026-01-15")).toEqual([
			"2026-01-15",
			"2026-01-15",
		]);
	});
