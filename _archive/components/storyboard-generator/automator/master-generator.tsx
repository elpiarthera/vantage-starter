...
export function MasterGenerator({
	onGenerate,
	onLoadDemo,
}: MasterGeneratorProps) {
	...
	const handleApprove = () => {
		if (generatedUrl) {
			onGenerate(
				generatedUrl,
				prompt || "Uploaded Storyboard Master",
				analyzedCount || 6,
			);
		}
	};
		...
		<div className="space-y-4 max-w-2xl mx-auto">
			<div className="text-center space-y-2 mb-8">
				<h2 className="text-section-title tracking-tight">
					Create Master Storyboard
				</h2>
				<p className="text-body">
					Generate a new storyboard or upload an existing master to begin the
					process.
				</p>
			</div>
							...
							>
								...
								<div className="flex flex-col items-center gap-2">
									...
									<h3 className="font-medium text-lg text-[var(--text-primary)]">
										Click to upload master storyboard
									</h3>
									<p className="text-sm text-[var(--text-secondary)]">
										Supports JPG, PNG, WEBP (rec. 3:2 ratio)
									</p>
								</div>
							</div>
