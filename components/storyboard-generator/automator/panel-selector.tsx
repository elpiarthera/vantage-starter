...
import { DEMO_FINAL_SEQUENCE } from "@/lib/storyboard-generator/demo-data";

// import { nanoid } from "nanoid"

// Helper function to replace nanoid
const generateId = () => Math.random().toString(36).substring(7);

// Helper function to replace toast
const toast = (opts: {
	title?: string;
	description?: string;
	variant?: string;
...
export function PanelSelector({
	...
	const loadDemoSequence = () => {
		const demoFinalPanels: FinalPanel[] = DEMO_FINAL_SEQUENCE.panels.map(
			(demoPanel) => {
		...
		<Card className="border-white/10 bg-black/40 backdrop-blur-sm shadow-2xl space-y-8 max-w-7xl mx-auto animate-in fade-in duration-500">
			<CardHeader className="pb-3">
				<CardTitle className="text-lg font-semibold">
					Build Your Final Storyboard
				</CardTitle>
				<CardDescription className="text-muted-foreground">
					Click panels to add them, or add a transition slot and populate it
					with any two images.
				</CardDescription>
				<details className="mt-3 text-xs bg-secondary/50 border border-border rounded-lg">
					...
					<div className="px-3 pb-3 text-muted-foreground">
						Avoid using copyrighted movie titles, character names, or
						trademarked content in your prompts. The AI content moderation may
						flag references like "Ratatouille", "Star Wars", etc. Instead, use
						generic descriptions like "animated chef story" or "space
						adventure".
					</div>
				</details>
			</CardHeader>
							...
							<div>
								...
								{customImages.length > 0 ? (
									...
									>
										...
										<p className="text-[var(--accent-text-muted)] text-xs">
				...
				<div className="space-y-4">
					<div className="flex items-center justify-between">
						<h3 className="text-xl font-bold">
							Final Sequence ({finalPanels.length})
						</h3>
						...
						{finalPanels.length === 0 ? (
							<div className="flex flex-col items-center justify-center h-64 border-2 border-dashed border-zinc-700 rounded-lg gap-4">
								<p className="text-zinc-500 text-sm">
									Click panels to build your sequence
								</p>
								<p className="text-zinc-600 text-xs">
									or add a keyframe pair for start→end animations
								</p>
							</div>
						) : (
							finalPanels.map((panel, index) => (
								...
								>
									...
									{/* Position */}
									<div className="text-sm font-mono text-zinc-400 min-w-[2ch]">
										{index + 1}
									</div>
									...
									{panel.type === "single" && panel.imageUrl ? (
										...
										<div className="flex gap-2 flex-1 items-center">
											...
											<div
												...
												onClick={() =>
													setSelectingFor({ panelId: panel.id, slot: "first" })
												}
											>
												{panel.imageUrl ? (
													<>
											...
											<div
												...
												onClick={() =>
													setSelectingFor({ panelId: panel.id, slot: "last" })
												}
											>
												{panel.linkedImageUrl ? (
													<>
