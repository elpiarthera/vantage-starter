...
import { DEMO_STORYBOARD } from "@/lib/storyboard-generator/demo-data";
...
export function PanelProcessor({
	masterUrl,
	masterPrompt,
	panelCount,
	storageMode,
	...
	const processPanels = async () => {
		...
		try {
			...
			if (storageMode === "temporal") {
				const dataUriCount = extractedPanels.filter((url) =>
					url.startsWith("data:"),
				).length;
				// toast.success(`${dataUriCount} panels ready (temporal - do not refresh page)`)
				console.log(`${dataUriCount} panels ready (temporal)`);
			} else {
				const httpUrlCount = extractedPanels.filter((url) =>
					url.startsWith("http"),
				).length;
				// toast.success(`${httpUrlCount} panels saved to persistent storage`)
				console.log(`${httpUrlCount} panels saved`);
			}
	...
	const loadDemoPanels = () => {
		...
		console.log(`Loaded ${demoPanelUrls.length} demo panels`);
		...
		<div className="space-y-6 max-w-4xl mx-auto">
			<div className="text-center mb-8">
				...
				<p className="text-[var(--text-secondary)]">
					Using Nano Banana to extract, upscale, and QC {panelCount} panels from
					the master.
					...
					<div className="space-y-2">
						<div className="flex justify-between text-sm">
							...
							<span className="text-[var(--text-primary)] font-medium capitalize flex items-center gap-2">
								...
								{status === "processing"
									? `Extracting Panel ${panels.length + 1}/${panelCount}...`
									: status}
							</span>
						</div>
