...
import { DEMO_TRANSITION_STORYBOARD } from "@/lib/storyboard-generator/demo-data";
...
export function TransitionGenerator({
	...
	const [status, setStatus] = useState<"ready" | "processing" | "complete">(
		"ready",
	);
	...
	const processPanels = async () => {
		...
		try {
			...
			if (storageMode === "temporal") {
				...
				console.log(
					`${extractedPanels.length} transition frames ready (temporal)`,
				);
			} else {
				// toast.success(`${extractedPanels.length} transition frames saved`)
				console.log(`${extractedPanels.length} transition frames saved`);
			}
	...
	const loadDemoExtractedPanels = () => {
		const demoPanelUrls = DEMO_TRANSITION_STORYBOARD.panels.map(
			(p) => p.imageUrl,
		);
		...
		console.log(`Loaded ${demoPanelUrls.length} demo transition frames`);
		...
		<div className="space-y-6 max-w-4xl mx-auto animate-in fade-in duration-500">
			<div className="text-center space-y-3 mb-8">
				...
				<p className="text-[var(--text-secondary)] max-w-2xl mx-auto">
					Your main storyboard contains intermediate transition states. Generate
					clean first/last frames for smooth transitions.
				</p>
			</div>
								...
								{isAnalyzing ? (
									...
									<>
										...
										{isEditingCount ? (
											...
											>
												<span>
													{analyzedCount ? `${analyzedCount} Panels` : "Ready"}
												</span>
