...
export function StoryboardPanel({
	...
	const handleEnhance = async () => {
		...
		setIsEnhancing(true);
		try {
			console.log("DEMO MODE: Enhance not available");
			...
			<div className="p-3 flex flex-col gap-3 bg-card flex-1">
				<div className="space-y-1.5">
					...
					<Select
						...
						onValueChange={(val) =>
							onUpdate(panel.id, { model: val as VideoModel })
						}
					>
						...
						<SelectContent>
							{isTransitionPanel ? (
								<>
									...
									<SelectItem value="veo3-standard">
										Veo 3.1 Standard
									</SelectItem>
					...
					<div className="flex items-center justify-between">
						...
						<div className="flex items-center gap-2">
							<Button
								...
								disabled={
									isEnhancing ||
									(!masterDescription.trim() && !panel.prompt.trim())
								}
							>
								{isEnhancing ? (
				...
				<div className="space-y-1.5">
					...
					<Select
						...
						onValueChange={(val) =>
							onUpdate(panel.id, {
								duration: Number.parseInt(val, 10) as 3 | 5 | 8,
							})
						}
					>
						<SelectTrigger className="h-7 text-xs">
				...
				{panel.error && (
					<div className="text-[10px] text-destructive mt-1 px-1">
						Error: {panel.error}
					</div>
				)}
			</div>
