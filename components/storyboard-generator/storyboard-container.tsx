"use client";
import { Film, Layers, Settings2, Wand2 } from "lucide-react";
...
export function StoryboardContainer({
	...
	useEffect(() => {
		if (initialPanels && initialPanels.length > 0) {
			...
			const savedVideoUrls =
				initialVideoUrls || /* savedSession?.videoUrls || */ {};

			const newPanels: StoryboardPanelData[] = initialPanels.map(
				(url, index) => ({
	...
	const generateAll = async () => {
		const pendingPanels = panels.filter(
			(p) => !p.videoUrl && !p.isGenerating && p.prompt.trim().length > 0,
		);

		if (pendingPanels.length === 0) {
			// toast.info("No ready panels to generate (check prompts)")
			console.log("No ready panels to generate (check prompts)");
			return;
		}

		// toast.info(`Starting generation for ${pendingPanels.length} panels...`)
		console.log(`Starting generation for ${pendingPanels.length} panels...`);

		await Promise.all(pendingPanels.map((p) => generateVideo(p.id)));
	};

	const loadDemoData = () => {
		console.log("loadDemoData called - demo data loading disabled");
		...
		return (
			<div className="w-full h-full flex flex-col items-center justify-center text-center bg-card">
				...
				<p className="text-xs text-muted-foreground max-w-xs mb-4">
					Add generated images from above to create a video storyboard powered
					by Veo.
				</p>
			</div>
		);
	...
	return (
		...
		>
				...
				<div className="flex-1 overflow-y-auto p-4 space-y-4">
					...
					{panels.length > 0 && panels.every((p) => !p.prompt) && (
						<div className="text-xs p-3 flex items-start gap-2 rounded-lg bg-muted text-muted-foreground">
							...
							<div>
								...
								<p>
									Your selected panels are ready. You can now add video prompts
									manually, use the "Load Demo Data" button to pre-fill example
									data, or enhance prompts using the master description and AI.
								</p>
							</div>
						</div>
					)}
					...
					<div className="rounded-lg p-4 bg-muted">
						...
						<p className="text-[10px] text-muted-foreground mt-3">
							...
							{panels.reduce((sum, p) => sum + (p.duration || 5), 0)}s
						</p>
					</div>
			...
			{/* Resize handle */}
			{/* biome-ignore lint/a11y/useSemanticElements: Custom interactive resize handle */}
			{/* biome-ignore lint/a11y/useFocusableInteractive: Drag handle uses mouse interactions */}
			<div
				role="separator"
			...
			<div className="flex-1 flex flex-col overflow-hidden bg-background">
				<div className="flex-shrink-0 h-12 px-4 flex items-center justify-between border-b border-border">
					...
					<span className="text-xs text-muted-foreground">
						{panels.filter((p) => p.videoUrl).length}/{panels.length} videos
						generated
					</span>
				</div>
						...
						{panels.length < 6 && (
							<div className="w-[280px] h-[420px] flex-none rounded-lg border border-dashed border-border bg-card flex flex-col items-center justify-center text-center p-6 opacity-50 hover:opacity-100 transition-opacity cursor-help">
								<p className="text-xs text-muted-foreground">
									Select an image above and click "Add to Storyboard"
								</p>
							</div>
						)}
					</div>
