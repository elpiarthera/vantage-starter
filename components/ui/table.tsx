 ...
 * repo's OKLCH theme (`border-border`, `bg-muted`, `bg-card`, `text-foreground`,
 * `text-background`, `text-muted-foreground`), so no color remapping was
 * needed. Wired into `components/chat/MessageList.tsx` (Batch 2,
 * docs/mcpcn-block-mapping.md §4 "table"): a new `table` message-part
 * renderer for the agent to emit an interactive, selectable table inline in
 * chat instead of a markdown wall of text. Replaces nothing — the chat
 * surface renders markdown tables today, not an interactive component.
 *
 * No shadcn `components/ui/table.tsx` existed in this repo before this port
 * (confirmed absent), so there is no naming collision to resolve.
 */
"use client";
	...
	<>
		{Array.from({ length: 3 }).map((_, row) => (
			<tr
				...
				// biome-ignore lint/suspicious/noArrayIndexKey: fixed-count (3) loading placeholder rows with no data identity of their own — never reordered, never added/removed individually.
				key={`skeleton-row-${row}`}
			>
				{Array.from({ length: columns }).map((__, column) => (
					<td
						className="px-3 py-3"
						// biome-ignore lint/suspicious/noArrayIndexKey: fixed-count loading placeholder cells, same reasoning as the row above.
						key={`skeleton-cell-${row}-${column}`}
					>
	...
	return (
		<td className="px-3 py-3">
			<button
				...
				className={cn(
					"flex size-4 items-center justify-center rounded border focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring",
					selected
