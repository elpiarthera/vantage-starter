...
// Two real consumers pass `asChild` (verified: `app/[locale]/admin/layout.tsx:212`
// and `components/dashboard/DashboardHeader.tsx:59`). Base UI's `Trigger` has no
// `asChild` prop — the wrapper keeps `asChild` in its own public surface and maps
// it to Base UI's `render` prop (the same asChild -> render mapping used on
// `AlertDialogTrigger`), so neither consumer needs to change.
