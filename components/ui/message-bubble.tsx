 ...
 *
 * Ported for VantageStarter: the upstream block targets Base UI's
 * `render` prop on its dropdown trigger; this repo's `dropdown-menu` is
 * Radix, so the reaction-picker trigger below uses Radix's `asChild`
 * instead — the only behavioural change from upstream. Colors are mapped
 * onto this repo's OKLCH tokens (`--primary`, `--muted`, `--foreground`,
 * `--card`, `--border`); no upstream `apps-sdk-theme` CSS is adopted.
 */
"use client";
