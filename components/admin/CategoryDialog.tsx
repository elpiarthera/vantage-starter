...
interface CategoryDialogProps {
	open: boolean;
	category: Doc<"toolCategories"> | null;
	metaCategories: Doc<"tools">[];
	defaultMetaCategoryId?: string;
	onClose: () => void;
	onSave: (data: {
...
export function CategoryDialog({
	...
	const t = useTranslations("admin.categories.dialog");
	const [formData, setFormData] = useState<{
		...
		toolId: (defaultMetaCategoryId as Doc<"tools">["_id"]) || "",
		key: "",
		name: "",
		nameTranslationKey: "",
		description: "",
	...
	useEffect(() => {
		if (category) {
			setFormData({
				...
				key: category.key,
				name: category.name,
				nameTranslationKey: category.nameTranslationKey,
				description: category.description || "",
				descriptionTranslationKey: category.descriptionTranslationKey || "",
	...
	}, [category, defaultMetaCategoryId]);

	const handleSubmit = (e: React.FormEvent) => {
		e.preventDefault();
		if (!formData.toolId) {
			alert(t("select_tool_error"));
			return;
		}
		onSave({
			...formData,
		...
		<Dialog open={open} onOpenChange={onClose}>
			<DialogContent className="w-full h-[100dvh] max-w-full top-0 left-0 translate-x-0 translate-y-0 rounded-none border-0 p-0 flex flex-col sm:max-w-full sm:h-[100dvh] sm:top-0 sm:left-0 sm:translate-x-0 sm:translate-y-0 sm:rounded-none sm:border-0 lg:top-[50%] lg:left-[50%] lg:translate-x-[-50%] lg:translate-y-[-50%] lg:h-auto lg:max-w-[720px] lg:max-h-[90vh] lg:rounded-lg lg:border lg:p-6">
				<div className="px-6 pt-6 lg:p-0 flex-shrink-0">
					<DialogHeader>
						<DialogTitle>
							{category ? t("edit_title") : t("create_title")}
						</DialogTitle>
						<DialogDescription>
				...
				<form onSubmit={handleSubmit} className="flex-1 flex flex-col min-h-0">
							...
							>
								...
								<SelectContent>
									{metaCategories.map((meta) => (
										<SelectItem key={meta._id} value={meta._id}>
											{meta.name}
										</SelectItem>
									))}
								</SelectContent>
					...
					<DialogFooter className="px-6 pb-6 lg:px-0 lg:pb-0 border-t lg:border-t-0 pt-4 lg:pt-0 flex-shrink-0 bg-background">
						...
						<Button type="submit">
							{category ? t("actions.save") : t("actions.create")}
						</Button>
					</DialogFooter>
				</form>
			</DialogContent>
