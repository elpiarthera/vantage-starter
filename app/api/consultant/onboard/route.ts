 ...
 * Output: text/event-stream (streamText → toTextStreamResponse)
 * Side effects:
...
export async function POST(req: Request) {
	...
	// 6. Load registry teams for context injection
	...
	// 7. Load conversation history if session exists
	let historyText = "";
	...
	// 10. Save user message BEFORE streaming (if session exists)
	if (project.sessionId) {
		await fetchMutation(
