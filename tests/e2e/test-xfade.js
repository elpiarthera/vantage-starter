...
// Run with: node tests/e2e/test-xfade.js
...
const SCENES = [
	"https://trustworthy-sparrow-452.convex.cloud/api/storage/3ca48676-f995-4b59-a2e3-95b230d0865e", // Scene 1
...
const TRANSITION_TYPE = "circleopen"; // Try: pixelize, wipeleft, slideup, fade, dissolve
...
async function runTest() {
	...
	// Offset 2 = Offset 1 + (Duration - Transition)
	const offset1 = CLIP_DURATION - TRANSITION_DURATION;
	const offset2 = offset1 + (CLIP_DURATION - TRANSITION_DURATION);

	console.log(`   Offset 1: ${offset1}s`);
	console.log(`   Offset 2: ${offset2}s\n`);

	// We use [0:v], [1:v], [2:v] inputs
	// Chain: 0+1 -> v1, then v1+2 -> out
	const filterComplex = `[0:v][1:v]xfade=transition=${TRANSITION_TYPE}:duration=${TRANSITION_DURATION}:offset=${offset1}[v1];[v1][2:v]xfade=transition=${TRANSITION_TYPE}:duration=${TRANSITION_DURATION}:offset=${offset2},format=yuv420p[out]`;
