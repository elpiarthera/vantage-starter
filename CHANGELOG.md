...
### Fixed (2026-07-16 — tsc 16 -> 0, two root causes, task k1756dw9ekbsstrf9cd1dtb0918anr77)

- **`npx tsc --noEmit` now returns 0.** It returned 16 for weeks, and every QA block on this repo — mine included — cited those 16 as a "pre-existing baseline" and moved on. That framing was the defect. This repo is **the boilerplate every fleet app is cloned from**: 16 tolerated errors here are 16 errors inherited by every future app. And "pre-existing" names a culprit who does not exist — every line here was written by this fleet. A red normalised is still a red; a debt *declared* is no better than a debt *hidden* when the rule is explicit.
- **16 errors were 2 causes, not 16 problems** — 15 + 1, derived per cause before touching anything:
  - **`vite` was never declared.** `grep -cE '"vite"' package.json` → **0**, `ls node_modules/vite` → absent — yet `vitest.config.ts:2` imports it and 13 `__tests__/convex/*` files declare `/// <reference types="vite/client" />`. **Third instance of the exact class that crashed the dashboard this morning**: `@ai-sdk/gateway` (4 importers, undeclared → backend never deployed) and `esbuild` (`scripts/build-litui.mjs:19`, undeclared). A package imported but undeclared works only while some transitive dependency happens to supply it.
  - **`@types/react` was a full major behind `react`.** `react: ^19` / `@types/react: ^18` (18.0.0 installed). `useOptimistic` is a React 19 API: `grep -c useOptimistic node_modules/@types/react/index.d.ts` → **0**. The component was never at fault — `components/chat/ModelSelector.tsx` is untouched, and no `as any` was used to silence it.
- **Version derived, not guessed** — and the obvious command was the wrong one: `npm view vite version` → **8.1.5**, which is *outside* the range vitest accepts (`npm view vitest@4.0.9 dependencies` → `vite: ^6.0.0 || ^7.0.0`). Pinned `^7.3.6` via `npm view vite@7 version` — the highest version actually satisfying the declared peer range.
- **`types/web-components.d.ts:15`** — bumping the types surfaced 6 further errors (`ui-button`, `lui-accordion*`), fixed at the root rather than suppressed: `declare global { namespace JSX }` → `declare module "react" { namespace JSX }`. React 19 moved the JSX namespace inside the `react` module, so global augmentation no longer merges.
- **`.github/workflows/quality.yml`** — the `TypeScript strict` gate is now a real step, and item 2 leaves the declared-absent list. It arrives **green on the commit that introduces it**, which is the whole condition for a gate to survive its first week. Bite probe on foreign material, landing grep-asserted **before** reading the verdict: `mv node_modules/vite node_modules/vite.disabled` → asserted absent → `tsc` → **15 errors, RED signature identical** → restored → 0.
- **One probe that did NOT bite, reported rather than dressed up**: downgrading `@types/react` back to `^18` reinstalls **18.3.31**, under which `tsc` returns **0** — the regression does not reproduce in the full dependency graph. So the RED came from the **lockfile pinning 18.0.0**, not from the `^18` range itself: the range was a permission, the lockfile was the fact. The gate is proven to bite on cause 1, **not** on cause 2, and that is written here rather than smoothed over. A probe that never reddens proves nothing, and claiming otherwise would be the exact forgery this file exists to prevent.
