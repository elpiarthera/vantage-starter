...
- **`settings.json` shipped clean** (sha256 `d4cd4b25677abeab`): the two dead wirings (`verify-breadcrumb.py`, `monitor-npm-version-vs-deployed.py`) and a corrupt entry (`"command": "$CLAUDE_PROJECT_DIRtrue"` — a broken variable substitution glued to `true`, which had survived every prior pass because nothing ever exercised it) are gone; the IRP matcher fix is in. A guard that is never invoked never turns red.
