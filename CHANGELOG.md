...
### Added (2026-07-16 — i18n guard, RED baseline: 211 hardcoded English literals across 70 files)

- **`scripts/i18n-guard.js`**: generic TypeScript-AST scanner (not a hardcoded string list) over `app/[locale]/dashboard/**` and every component root it consumes (`components/missions`, `components/chat`, `components/create`, `components/design-system`, `components/dashboard`). Detects hardcoded English JSX text nodes, `aria-label`/`aria-description`/`placeholder`/`title`/`alt` string-literal attributes, and hardcoded locale tags (e.g. `"en-US"`, `"en-GB"`) passed to `toLocaleDateString`/`toLocaleString`/`Intl.DateTimeFormat`. Fails loud (exit 2) if it scans 0 files — no silent clean bill of health. Run via `npm run i18n:guard`.
- **RED baseline** (this commit, PR #12's follow-up scope): `node scripts/i18n-guard.js` reports **211 violations across 70 scanned files**, including every string Laurent saw in prod screenshots — `"Mission Control"`, `"See the factory floor"`, `"New chat"`, `"Describe what you want to build"`, `"Sessions"`, `"New session"` — proving PR #12 closed one page while the class of 24/33 zero-`useTranslations` files stayed open.
- **`scripts/__tests__/i18n-guard.test.js`**: bipolar probe. MUST_BLOCK re-injects a hardcoded English sentence into `app/[locale]/dashboard/architect/_components/session-list.tsx` (real foreign material, not scanner-authored fixture), grep-asserts the mutation landed before reading any verdict, asserts the guard turns RED naming the injected string, restores the file and asserts `git diff --stat` is empty. MUST_PASS asserts product-name/single-token identifiers (`Architect`, `Chat`, `Mosaic`) are never flagged — 0 false positives.
