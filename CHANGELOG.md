...
- **Choke-point vs call-site decision**: `getByPolarProductId` has exactly two callers, both in `convex/http.ts` — `order.paid` (a grant path) and `subscription.updated` (tier bookkeeping only, not a grant). Filtering `isActive` at the choke point also affects the bookkeeping caller: a subscription pointing at a now-disabled product will no longer have its `tierKey` updated, logging `"Unknown productId — tier not found in DB"` instead. This is accepted as correct behavior — a disabled tier should not be newly assigned to any subscription either — and matches the pre-existing (previously failing) security test that asserts `getByPolarProductId` returns `null` for `isActive: false`.
- **Tests**: `__tests__/convex/polar-security.test.ts` — 2 new tests added reproducing the full grant paths (`order.paid` via `getByPolarProductId` + `addPurchaseCredits`, and the renewal path via `addMonthlyRenewalCreditsFixed`), both proven RED before the fix (verbatim in PR), plus a pre-existing failing test in the same file that is now fixed as a side effect. Full Convex suite: 178/188 passing after fix (175/188 before) — the 10 remaining failures are pre-existing and unrelated (missing `.env.local` Polar product env vars, an unrelated watch-page test).
